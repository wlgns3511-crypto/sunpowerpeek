    ...
    <>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://sunpowerpeek.com/blog/${slug}/`,
            },
          }),
        }}
      />
      ...
      <header className="mb-8">
        ...
        <div className="text-xs text-slate-400 flex items-center gap-3">
          ...
          <span>· SunPowerPeek Editorial Team</span>
        </div>
      </header>
      ...
      <div className="mt-12 p-6 bg-orange-50 border border-orange-200 rounded-xl">
        ...
        <p className="text-slate-600 text-sm mb-4">
          Use our free tools to calculate your solar ROI and compare solar
          potential across states.
        </p>
        <div className="flex flex-wrap gap-3">
          <a
            href="/calculator/"
            className="text-sm px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 font-medium"
          >
            Solar Calculator
          </a>
          <a
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {suggestions.map((p) => (
              <a
                key={p.slug}
                href={`/blog/${p.slug}/`}
                className="block border border-slate-200 rounded-xl p-4 hover:border-orange-300 hover:shadow-sm transition-all group"
              >
                <span className="text-xs text-orange-600">{p.category}</span>
                <h3 className="text-sm font-semibold text-slate-900 group-hover:text-orange-700 mt-1 leading-snug">
                  {p.title}
                </h3>
